...
As you can see Linked Lists allow you to add your values in anywhere because you just have to change the next pointer of the previous node to the new node, and the previous pointer of the next node to the new node.
...
Now that you know what a Linked List is, it is important to understand why you would use a Linked List instead of other datastructures.
**1. Mutability**
...
**Pointers**
Pointers are what make Linked Lists work. These are simple refrences that show what the previous node is and what the next node is. This is what allows us to iterate throught the list.

**The Head**
The Head of a Linked list is the first node in the list. It contains only a forward pointer as there is nothing before it.

**The Tail**
The Tail of a Linked List is the last node in the list. It contains only the previous pointer as there is nothing after it. In a Linked List with only 1 node, that node is simultaneaously the Head and Tail.
...
class LinkedList:
	class Node():
		...
		self.prev = None
```
Then we makes the constructor. This is what is run when a new instance of Linked List is created. The Linked List object has two member values, the Head and the Tail nodes. These are initially None since we do not have Nodes stored yet.
```python
class LinkedList:
	class Node():
	    ... # To save space the node class will be replaced with elipses.
    def __init__(self)
        self.head = None
        self.tail = None
```
...
if self.tail is None:
	...
	self.tail = new_node
```
Now that we have methods to insert heads and tails, we must also make a method to remove heads and tails. Lets start with remove_head.

remove_head operates very simply. First it check to make sure the list isn't only 1 node. If it is then both the head and tail values of the list need cleared since the single node is both the head and tail.
```python
def remove_head(self):
	if self.head == self.tail:
		...
		self.tail = None
```
Once that is completed we can tell it what to do if the head isn't already none. The current head's next node has it's previous value set to None since it will be the new head. Then the Linked List head is set to be the old head's next node.
```python
def remove_head(self):
	if self.head == self.tail:
		self.head = None
...
def remove(self. value):
	...
	while curr is not None:
		...
		curr = curr.next
```
Another useful method for our linked list is the replace method which will allow us to replace the data in a node rather than having to delete it along with it's pointers before creating it again.
...
def __iter__(self):
	...
	while curr is not None:
		...
		curr = curr.next
```
We can also include the reversed function to do the same thing but backwards.
```python
def __reversed__(self):
	...
	while curr is not None:
		...
		curr = curr.prev
```
The last method you should include in your linked list is the string method. This allows you to display a string representaion of the linked list which is useful for debugging. This method also uses the iteration method we made above in the for loop.
```python
def __str__(self):
	outpur = linkedlist[]
	first = True
